...
void setup() {
  ...
  for(int yy=0; yy<2; yy++){
    ...
    for(int xx=0; xx<4; xx++){   
       elements[i]=new ClayblockXYA(plotX, plotY, xySide, xySide, cRadius, "/xy"+(xyToken++));
       i++;
       elements[i]=new ClayblockRibbon(plotX, plotY+ribbonSpacing+xySide, xySide, ribbonHeight, "/r"+(rToken++) );
       i++;
       plotX+=xySide+offset;
    }
    plotX=vertBorder;
